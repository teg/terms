...
\begin{itemize}
	...
	\item $\llbracket u\terdel{v}\rrbracket=\llbracket u\rrbracket$; and
...
\begin{definition}
	...
	\[\tempty:=\epsilon\;|\;\tempty\terdel{t}\;|\;\tempty[x,y\leftarrow t]\;.\]
\end{definition}
...
\begin{itemize}
	...
	\item $u\terdel{t[x,y\leftarrow v]}\rightsquigarrow u\terdel{t}[x,y\leftarrow v]$; and
	\item $u\terdel{x}[x,y\leftarrow t]\rightsquigarrow u\{y\leftarrow t\}$.
\end{itemize}
...
\begin{definition}
	The \emph{weight} of a deletion, $u\terdel{t}$, (resp, of a duplication, $u[x,y\leftarrow t]$) in a term $v$ is defined to be the \emph{size} of $t$ in $v$, $|t|_v$:
	\begin{itemize}
		\item $|w|_v$, if $t$ is $w\terdel{w'}$ (resp., $w[x,y\leftarrow w']$);
...
\begin{definition}
	...
	\[\tpair:=\terpair{u}{v}\;|\;\tpair\terdel{t}\;|\;\tpair[x,y\leftarrow t]\;.\]
\end{definition}

\paragraph{Abstraction Reductions}

\begin{itemize}
	\item $u[z_1,z_2\leftarrow\lambda y.t]\rightsquigarrow u[z_1,z_2\leftarrow\lambda y.\terpair{x_1}{x_2}[x_1,x_2\leftarrow t]]$, if $t$ is not a $\tpair$;
	\item $\lambda x.(u[y_1,y_2\leftarrow y])\rightsquigarrow (\lambda x.u)[y_1,y_2\leftarrow y]$, if $x\neq y$;
	\item $u[x_1,x_2\leftarrow(\lambda y.\terpair{t_1}{t_2}[y_1,y_2\leftarrow y])]\rightsquigarrow u\{x_1\leftarrow\lambda y_1.t_1,x_2\leftarrow\lambda y_2.t_2\}$, where $y_1\in\FV(t_1)$ and $y_2\in\FV(t_2)$; and
	\item $u[x_1,x_2\leftarrow(\lambda y.\terpair{t_1}{t_2}\terdel{y})]\rightsquigarrow u\{x_1\leftarrow\lambda y.t_1\terdel{y},x_2\lambda y.t_2\terdel{y}\}$.
\end{itemize}

\paragraph{Substitution Reductions}

\begin{itemize}
	\item $u[x,y\leftarrow t\terdel{v}]\rightsquigarrow u[x,y\leftarrow t]\terdel{v}$; and
...
\begin{itemize}
	...
	\item $u[x,y\leftarrow\terpair{t_1}{t_2}]\rightsquigarrow u\{x\leftarrow t_1,y\leftarrow t_2\}$.
\end{itemize}
...
\begin{lemma}
	$u[x,y\leftarrow t]\rightsquigarrow^\star u\{x\rightarrow t',y\rightarrow t''\}[x_1,y_1\leftarrow(v_1)w_1]\cdots[x_m,y_m\leftarrow(v_m)w_m][x_{m+1},y_{m+1}\leftarrow z_{m+1}]\cdots[x_n,y_n\leftarrow z_n]=terdel{z_{n+1}}\cdots\terdel{z_p}$,
...
\begin{proof}
	The theorem follows trivially for the deletion reductions, as $\llbracket u\terdel{v}\rrbracket = \llbracket u\rrbracket$.

The casees for the duplication reductions follows by a straight-forward case analysis.
\end{proof}

\begin{theorem}
	If\/ $\llbracket u\rrbracket \rightsquigarrow_\beta \llbracket v \rrbracket$, then $u'\rightsquigarrow_\beta v'$, where $\llbracket u'\rrbracket = \llbracket u \rrbracket$ and $\llbracket v \rrbracket = \llbracket v'\rrbracket$.
\end{theorem}

\begin{proof}
	Let $u'$ be the normal form of $u$ with respect to the deletion and duplication reductions. It is straight-forward to verify by a case-analysis that $u'$ contains the same $\beta$-redex as $\llbracket u'\rrbracket$, so $v'$ is the corresponding contractum.
\end{proof}
